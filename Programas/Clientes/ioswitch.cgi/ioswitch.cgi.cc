...
int main(int /*argc*/, char** /*argv*/, char** env)
{
  ...
  if( pConfig->GetParam("TRACE-IOSWITCH.CGI", s_trace))
  {
    trace = atoi(s_trace);
  }
  ...
  if(trace)
  {
    openlog("ioswitch.cgi", 0, LOG_USER);
  ...
  if(trace)
  {
    syslog(LOG_DEBUG, "Call Q: ioswitch [%s]", query.C_Str());
  }

  rc = pClient->Call("dompi_pi_ioswitch", query, response, 100);
  if(rc == 0)
  {
    if(trace)
    {
