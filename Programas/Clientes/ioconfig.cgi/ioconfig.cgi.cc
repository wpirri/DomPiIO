...
int main(int /*argc*/, char** /*argv*/, char** env)
{
  ...
  if( pConfig->GetParam("TRACE-IOCONFIG.CGI", s_trace))
  {
    trace = atoi(s_trace);
  }
  ...
  if(trace)
  {
    openlog("ioconfig.cgi", 0, LOG_USER);
  ...
  if(trace)
  {
    syslog(LOG_DEBUG, "Call Q: ioconfig [%s]", query.C_Str());
  }

  rc = pClient->Call("dompi_pi_ioconfig", query, response, 100);
  if(rc == 0)
  {
    if(trace)
    {
